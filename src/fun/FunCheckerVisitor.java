...
public class FunCheckerVisitor extends AbstractParseTreeVisitor<Type> implements FunVisitor<Type> {
	...
	public Type visitFor(FunParser.ForContext ctx) {
		//check type of the control variable
		define(ctx.ID().getText(), Type.INT, ctx);
		// visit expression  e1 and e2 to retrieve type in type table
		Type t1 = visit(ctx.e1);
		Type t2 = visit(ctx.e2);
		//visit the sequential command
		visit(ctx.seq_com());												//EXTENSION
		// checkType checks that t1  and t1 are of the type INT.
	...
	public Type visitSwitch(FunParser.SwitchContext ctx) {                                //EXTENSION
		//visit expr to retrieve its type
		Type expr_type = visit(ctx.expr());
		//check type
		if (expr_type != Type.BOOL && expr_type != Type.INT) {
			reportError("The types are not valid in the fun language", ctx);
		}
		List<FunParser.GuardContext> guards = ctx.guard();
		List<String> visitedGuards = new ArrayList<String>();
		// loop in the guards parser(number of cases in the switch) 
		for (FunParser.GuardContext guard : guards){
			//visit each guard
			Type guard_type = visit(guard);
			checkType(guard_type, expr_type, ctx);
			// check if guards are repeating or overlapping
			if (visitedGuards.contains(guard.getText())){
				reportError("Overlapping guards", ctx);
			} 
			else if (guard_type == Type.BOOL) {
				//check type and add the text in the list
				checkType(guard_type, expr_type, ctx);
				visitedGuards.add(guard.getText());
			} 
			else {
				// regex to check that digits are in the range 0-9 and the next digit is more than the preceding digit
				//valid for cases like 2..4, 5..8
				if (guard.getText().matches("\\d+")) {
					checkType(guard_type, expr_type, ctx);
					visitedGuards.add(guard.getText());
				} 
				// error if there is overlapping such as 3..2, 8..4
				else {
					...
					if (i >= j) {
						reportError("first value in range should be the smaller", ctx);
					}
	 ...
	 * @return the visitor result
	 */
	//visit guard NUM DOTS NUM
	public Type visitRangeg(FunParser.RangegContext ctx) {								// EXTENSION								// EXTENSION
		return Type.INT;
	}
